...
const AppContent = () => {
  const { backendReady, setBackendReady, isAuthenticated } = useAuth();
  const location = useLocation();

  // Skip startup page for public routes
  const isPublicRoute = PUBLIC_ROUTES.includes(location.pathname);

  // Show startup page only for protected routes when backend isn't ready
  if (!backendReady && !isPublicRoute && isAuthenticated) {
